 ...
 */
function makeHappy(arrayToMakeHappy){
  return arrayToMakeHappy.map(
    (element) => {return "Happy "+element;}
  );
...
function getFullAddressesOfProperties(arrayOfAddresses){
  return arrayOfAddresses.map(
    (element) =>{return element.streetNumber+" "+element.streetName+" "+element.streetType+" "+element.city+" "+element.state+" "+element.zip; }
  );
 ...
 */
function findLargest(arrayElements){
          let largest = arrayElements [0];
              arrayElements.forEach(
                (element) => {
                  if(typeof arrayElements[arguments]=='string')
                  {
                    largest[arguments] = element[arguments]
                  }
                else
                {
