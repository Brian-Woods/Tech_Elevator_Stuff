...
namespace Exercises
{
    public partial class Exercises
    {
        ...
        public Dictionary<int, int> IntCount(int[] ints)
        {
            ...
            foreach (int number in ints.ToList())
            {
                int countOf;


                var containsKey = intCount.TryGetValue(number, out countOf);
                if (containsKey)
                {
                    var newCount = intCount[number] + 1;
                    intCount[number] = newCount;
                }
                else
                {
                    intCount.Add(number, 1);
                }
