...
-- name. 
-- (19 rows)
SELECT population, (district+', '+name) AS name_and_state
FROM city
...
SELECT TOP 1 population AS smallest_country_population
FROM country
WHERE population > 0
...
SELECT abs(gnp-gnpold) as ABS_Differece, name
FROM country
WHERE abs(gnp-gnpold)> 0
ORDER BY ABS_Differece ASC

-- 17. The average population of cities in each country (hint: use city.countrycode)
-- ordered from highest to lowest.
-- (highest avg population: 4017733.0000, "SGP")
	
SELECT  avg(population) AS av_pop, countrycode
FROM city
GROUP BY countrycode
ORDER BY av_pop DESC
...
SELECT count(name) as NUM_Cities, countrycode
FROM city
GROUP BY countrycode
...
SELECT max(population) as MAXIMUM, countrycode
FROM city
GROUP BY countrycode
...
SELECT avg(lifeexpectancy) AS Average_Life, min(lifeexpectancy) AS Min_Life, max(lifeexpectancy) AS Max_Life, continent
FROM country
